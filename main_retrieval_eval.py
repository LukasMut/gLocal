...
def evaluate_normal_vs_transformed(embeddings_dir, data_root, transform_path, update_transforms=False,
    ...
    for model_name in tqdm(CLIP_MODELS):
        ...
        if update_transforms:
            ...
            image_transformed = torch.tensor(things_feature_transform.transform_features(embeddings['images'],
                                                                                         model_name=transform_model_key))
            text_transformed = torch.tensor(things_feature_transform.transform_features(embeddings['text'],
                                                                                        model_name=transform_model_key))
            np.savez(os.path.join(embeddings_dir, f'{model_name}.npz'),
                     images=embeddings['images'], text=embeddings['text'],
                     image_transformed=image_transformed, text_transformed=text_transformed)
        else:
        ...
        if concat_weight is not None:
            ...
            image_transformed = torch.cat(image * (1 - concat_weight),
                                          image_transformed * concat_weight, dim=1)
            text_transformed = torch.cat(text * (1 - concat_weight),
                                         text_transformed * concat_weight, dim=1)

        # Evaluate with transforms
        results_t = evaluate(image_transformed, text_transformed, dataset_root=data_root)
