...
class ContrastiveLoss(nn.Module):
    ...
    @staticmethod
    def mask_diagonal(dots: Tensor) -> Tensor:
        return dots[~torch.eye(dots.shape[0], dtype=bool)].reshape(dots.shape[0], -1)

    def get_teacher_distribution(self, teacher_similarities: Tensor) -> Tensor:
        return F.softmax(self.mask_diagonal(teacher_similarities), dim=-1)
